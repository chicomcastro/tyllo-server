...
import { ClientEvents, ServerEvents } from "../../../src/events";
import { expect } from "chai";
import { InMemoryTodoRepository } from "../../../src/modules/Todo/todo.repository";
  ...
  describe("list todo", () => {
    it("should return a list of entities", (done) => {
      ...
      socket.emit("todo:list", (res) => {
        ...
        expect(res.data[0].title).to.equal("lorem ipsum");
        expect(res.data[0].completed).to.equal(false);
        expect(res.data[1].title).to.equal("dolor sit amet");
        expect(res.data[1].completed).to.equal(true);
        done();
      });
    });
  });
